...
const ScheduledInstagramAppPage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(API_ACCOUNT_POSTS + "?allowed=true&published=false", {
                method: 'GET',
    ...
    const handleDeletePostClick = async (id) => {
        ...
        try {
            ...
            navigate("/app/instagram/scheduled?notify=Post%20deleted%20correctly&notify-type=success");
            window.location.reload();
        } catch (error) {
            navigate("/app/instagram/scheduled?notify=The%20post%20could't%20be%20deleted&notify-type=error");
            window.location.reload();
        }
    };
    ...
    return (
        <App>
            <h1 className="text-white text-2xl font-bold mb-8">Scheduled</h1>
...
export default ScheduledInstagramAppPage;
